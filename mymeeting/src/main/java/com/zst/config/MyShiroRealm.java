...
import com.zst.service.impl.SysRoleService;
import com.zst.service.impl.SysUserService;
...
public class MyShiroRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        SysUser user = permissionService.findByUserEmail(token.getUsername());
        if (user != null){
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        SysUser user = permissionService.findByUserEmail(loginName);
        logger.info("权限认证!");
        if (user != null){
            // 权限信息对象info，用来存放查出的用户的所有的角色及权限
            SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
            //用户的角色集合
            info.setRoles(roleService.getRolesName(user.getId()));
